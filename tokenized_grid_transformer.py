...
class ARCTokenizer:
    def __init__(self):
        self.vocab_size = 10**4  # 10,000 possible tokens (10x10x10x10)
        self.token_to_grid = {i: self._index_to_grid(i) for i in range(self.vocab_size)}
    ...
    def tokenize(self, grid):
        ...
        for i in range(0, h - 1, 2):
            for j in range(0, w - 1, 2):
                ...
                token = self.grid_to_token[tuple(subgrid.flatten())]
                tokens.append(token)
        return tokens
    ...
    def pad_grid(self, grid):
        ...
        new_h = ((h + 1) // 2) * 2
        new_w = ((w + 1) // 2) * 2
        padded_grid = np.zeros((new_h, new_w), dtype=int)
        padded_grid[:h, :w] = grid
        return padded_grid

class TokenizedGridTransformer(nn.Module):
    def __init__(self, num_layers, embed_dim, num_heads, ff_dim, max_seq_length):
        super().__init__()
